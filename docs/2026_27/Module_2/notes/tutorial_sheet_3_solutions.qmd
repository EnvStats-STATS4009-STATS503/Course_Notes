...
1. What value of `P` (i.e., the period) was used in this model? 
2. State the fitted regression equation
3. Calculate the amplitude of the seasonal cycle.

`r hide("Solution")`

1. P = 12 months —  data is monthly and one full annual cycle spans 12 observations.
2. $\widehat{\text{temp}}_t = 12.430 - 4.216\cos(2\pi t/12) + 2.850\sin(2\pi t/12)$
3. $\sqrt{(-4.216)^2+2.85^2} \approx 5.09.$
...
where $(x - c)_+ = \begin{cases} 0 & \text{if } \ x < c\\ x- c & \text{if } x \geq c \end{cases}$

Show that 

1.  When $x_i < c$, $\mathbb{E}(Y_i) = \alpha_1 + \beta_1x_i$
2.  When $x_i \geq c$, $\mathbb{E}(Y_i) =  \alpha_2 + \beta_2 x_i$ such that $\alpha_2 = \alpha_1 + (\beta_1 - \beta_2)c$


`r hide("Solution")`

1. when $x_i < c$, $\mathbb{E}(Y_i) = \alpha_1 + \beta_1x_i - (\beta_1-\beta_2)\overbrace{(x_i-c)_+}^{0}  = \alpha_1 + \beta_1 x_i$
2. When $x_i \geq c$, 
