...
\section{Estimator properties}\label{estimator-properties}
...
\begin{tcolorbox}[enhanced jigsaw, title={Task 1}, toptitle=1mm, colframe=quarto-callout-important-color-frame, colbacktitle=quarto-callout-important-color!10!white, arc=.35mm, bottomtitle=1mm, bottomrule=.15mm, leftrule=.75mm, opacitybacktitle=0.6, coltitle=black, breakable, rightrule=.15mm, colback=white, toprule=.15mm, titlerule=0mm, left=2mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, title={Task 2}, toptitle=1mm, colframe=quarto-callout-important-color-frame, colbacktitle=quarto-callout-important-color!10!white, arc=.35mm, bottomtitle=1mm, bottomrule=.15mm, leftrule=.75mm, opacitybacktitle=0.6, coltitle=black, breakable, rightrule=.15mm, colback=white, toprule=.15mm, titlerule=0mm, left=2mm, opacityback=0]
...
\item
  ...
  unordered sample which is \textbf{unbiased} and \textbf{imprecise} as
  long as there is no systematic pattern in how observation are
  collected.
\item
...
(MLEs)}\label{maximum-likelihood-estimators-mles}

In general, statistical inference is based on the idea that the data we
observe are outcomes of random variables characterised by a distribution
\(f(\cdot)\) with parameters \(\theta\) that we would like to learn from
data.

Suppose we observe multiple random variables \(y_1, \dots, y_n\),
modelled as independent draws from \(f(y \mid \theta)\). Under this
independence assumption, their joint distribution is the product
\(\prod_{i=1}^{n} f(y_i \mid \theta)\). Then, the \textbf{likelihood} is
the joint distribution of the observable random variables, written as a
function of the parameter \(\theta\):

\[
L(\theta;\mathbf{y}) = \prod_{i=1}^{n} f(y_i \mid \theta),
\qquad
\ell(\theta) = \log L(\theta;\mathbf{y}) = \sum_{i=1}^{n}\log f(y_i \mid \theta).
\] Essentially, the likelihood tells us how plausibly each candidate
value of \(\theta\) is, based on the data we have. The value of
\(\theta\) that maximises the likelihood is the \textbf{maximum
likelihood estimate} (MLE) \(\hat\theta\). For practical reasons, we
often maximize the log-likelihood \(\ell'(\theta) = 0\) which is
equivalent to maximizing the likelihood. The MLE is then found by
solving \(\ell'(\theta) = 0\), and many familiar estimators (including
the sample mean) turn out to be maximum likelihood estimators.

The great practical advantage is that, \textbf{in large samples}, the
maximum likelihood estimator is approximately Normal,

\[
\hat\theta \;\dot\sim\; N\!\left(\theta,\; \frac{1}{I(\theta)}\right),
\qquad
I(\theta) = -\,\mathbb{E}\!\left[\frac{\partial^2 \ell}{\partial\theta^2}\right],
\]

where \(I(\theta)\) is the \textbf{Fisher information} which is the
matrix of second derivatives of the log-likelihood with respect to the
parameter of interest \(theta\) evaluated at the MLE (because \(\theta\)
is unknown). Notice that the asymptotic variance-covariance matrix
\(\text{Var}(\hat\theta)\) of the MLE is the inverse of the Fisher
information. Thus, the standard error is therefore
\(\text{se}(\hat\theta) = \sqrt{\text{Var}(\hat\theta)}\), and a 95\%
\textbf{Wald interval} is

\[
\hat\theta \;\pm\; 1.96\,\text{se}(\hat\theta).
\]

These estimators are \emph{consistent}, \emph{asymptotically Normal},
and \emph{efficient} --- in large samples no consistent estimator has a
smaller variance. Efficiency is the formal counterpart of the
\textbf{precision} idea illustrated above.
